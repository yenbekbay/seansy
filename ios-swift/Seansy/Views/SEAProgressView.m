...
- (instancetype)initWithFrame:(CGRect)frame {
  ...
  self.spinnerHeight = CGRectGetHeight(self.bounds);
...
- (void)layoutSubviews {
  ...
  self.backgroundLayer.anchorPoint = CGPointMake(0.5f, self.spinnerHeight / CGRectGetHeight(self.bounds) / 2);

  UIBezierPath *path = [UIBezierPath bezierPath];
  path.lineCapStyle = kCGLineCapButt;
  path.lineWidth = self.lineWidth;

  CGPoint center = CGPointMake(CGRectGetWidth(self.bounds) / 2, self.spinnerHeight / 2);
...
- (void)setSpinnerHeight:(CGFloat)spinnerHeight {
  ...
  self.backgroundLayer.frame = self.bounds;
  self.backgroundLayer.anchorPoint = CGPointMake(0.5f, spinnerHeight / CGRectGetHeight(self.bounds) / 2);
}
