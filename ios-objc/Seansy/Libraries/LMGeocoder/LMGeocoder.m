...
#define kGoogleAPIReverseGeocodingURL(lat, lng) [NSString stringWithFormat:@"http://maps.googleapis.com/maps/api/geocode/json?latlng=%f,%f&sensor=true&language=ru", lat, lng];
#define kGoogleAPIGeocodingURL(address)         [NSString stringWithFormat:@"http://maps.googleapis.com/maps/api/geocode/json?address=%@&sensor=true&language=ru", address];
#define kTimeoutInterval 60

@interface LMGeocoder ()

@property (nonatomic) BOOL isReverseGeocoding;
@property (nonatomic) LMGeocoderService currentService;

@property (nonatomic) CLLocationCoordinate2D requestedCoordinate;
@property (nonatomic) NSString *requestedAddress;
